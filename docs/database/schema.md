...
| —             | `plan`              | enum         | `free` / `player_pro` / `club` / `liga` / `federation` — default `free` |
| —             | `plan_expires_at`   | timestamp    | nullable — `null` = plano sem expiração (free ou vitalício)             |
| —             | `slug`              | varchar(30)  | nullable — URL amigável `@slug`; único; só exibido se Player Pro+       |
...
| -------------- | ---------- | ----------- | ------------------------------------------ |
| `modcodigo`    | `id`       | bigint      |                                            |
| —              | `key`      | varchar(60) | slug único — ex: `campo`, `society`         |
| `moddescricao` | `name`     | varchar(45) | nome exibido — ex: "Campo", "Society"       |
...
| -------------- | ---------- | ----------- | ------------------------------------------ |
| `catcodigo`    | `id`       | bigint      |                                            |
| —              | `key`      | varchar(60) | slug único — ex: `livre`, `sub_15`         |
| `catdescricao` | `name`     | varchar(45) | nome exibido — ex: "Livre", "Sub-15"       |
...
| ------------- | -------------- | ----------- | ---------------------------------- |
| `poscodigo`   | `id`           | bigint      |                                    |
| —             | `key`          | varchar(60) | slug único — ex: `lateral_direito`  |
...
| -------------- | ---------- | ----------- | ------------------------------------ |
| `forcodigo`    | `id`       | bigint      |                                      |
| —              | `key`      | varchar(30) | slug único — ex: `4-4-2` (= name)    |
...
| —                 | `team_invitations`   | Convite de elenco pendente de aceite do atleta      |
...
| —                    | `is_active`   | boolean      | default `true` — soft delete para times abandonados |
...
#### `team_invitations` (novo — convite de elenco)

| Campo                | Tipo         | Observação                                       |
| -------------------- | ------------ | ------------------------------------------------ |
| `id`                 | bigint       | PK                                               |
| `team_sport_mode_id` | bigint FK    | FK → `team_sport_modes`                          |
| `invited_user_id`    | bigint FK    | FK → `users` — usuário convidado                 |
| `invited_by`         | bigint FK    | FK → `users` — dono do time que enviou o convite |
| `position_id`        | bigint FK    | nullable — FK → `positions` (posição proposta)   |
| `status`             | enum         | `pending` / `accepted` / `rejected` / `expired`  |
| `expires_at`         | timestamp    | quando o convite expira automaticamente          |
| `message`            | varchar(255) | nullable — mensagem opcional do dono             |

> O convite usa `invited_user_id` (FK → `users`) e não `player_id`: o usuário convidado pode ainda não ter perfil de jogador completo. Ao aceitar, o sistema cria o `player_memberships` correspondente.
> Status `expired` é definido por job scheduler que compara `expires_at` com `NOW()`.
...
erDiagram
    ...
    team_sport_modes ||--o{ team_invitations : "invites to"
...
| —                | `category_id` | bigint FK    | nullable — FK → `categories` (faixa etária, ex: Sub-17)                   |
...
| —                       | `home_penalties` | int                                                   | nullable — gols em pênaltis (mata-mata encerrado em empate) |
...
erDiagram
    ...
    categories ||--o{ championships : "categorizes"
...
| `amiconfimacao`          | `confirmation`         | enum         | `pending` / `confirmed` / `rejected` / `expired`                  |
| —                        | `invite_expires_at`    | timestamp    | nullable — quando o convite expira (padrão: +2 dias após criação) |
...
| `label_key`   | varchar(150) | chave de tradução — ex: `badges.golden_ball.label`  |
| `description_key` | varchar(150) | chave de tradução — ex: `badges.golden_ball.description` |
| `icon`        | varchar(100) | icon key neutra — ex: `award`, `trophy`, `star`     |
| `scope`       | enum         | `championship` / `friendly` / `career` / `seasonal` |

**Catálogo inicial de badges:**

| `name`               | `label_key`                          | `scope`        | Critério                                                         |
| -------------------- | ------------------------------------ | -------------- | ---------------------------------------------------------------- |
| `golden_ball`        | `badges.golden_ball.label`           | `championship` | Mais votos de MVP num campeonato (`championship_awards`)         |
| `top_scorer`         | `badges.top_scorer.label`            | `championship` | Maior número de gols num campeonato                              |
| `best_assist`        | `badges.best_assist.label`           | `championship` | Maior número de assistências num campeonato                      |
| `best_goalkeeper`    | `badges.best_goalkeeper.label`       | `championship` | Goleiro da equipe com menor média de gols sofridos no campeonato |
| `fair_play`          | `badges.fair_play.label`             | `championship` | Zero cartões durante todo o campeonato                           |
| `hat_trick`          | `badges.hat_trick.label`             | `career`       | Marcou 3+ gols em uma única partida (amistoso ou campeonato)     |
| `iron_man`           | `badges.iron_man.label`              | `championship` | Participou de 100% das partidas de um campeonato                 |
| `unbeaten_champion`  | `badges.unbeaten_champion.label`     | `championship` | Conquistou o título sem perder nenhuma partida                   |
| `top_scorer_season`  | `badges.top_scorer_season.label`     | `seasonal`     | Maior total de gols somando campeonatos + amistosos na temporada |
| `best_assist_season` | `badges.best_assist_season.label`    | `seasonal`     | Maior total de assistências na temporada                         |
| `mvp_streak`         | `badges.mvp_streak.label`            | `career`       | Ganhou MVP em 3 ou mais partidas consecutivas                    |
| `loyal_player`       | `badges.loyal_player.label`          | `career`       | Participou de 5+ campeonatos pelo mesmo time                     |
| `rising_star`        | `badges.rising_star.label`           | `seasonal`     | Destaque de desempenho em sua primeira temporada completa        |
| `clean_sweep`        | `badges.clean_sweep.label`           | `championship` | Venceu todas as partidas da fase de grupos                       |
...
| `year`            | int       | nullable — ano-calendário; obrigatório quando `scope = seasonal` |

> Um jogador pode acumular o mesmo badge em campeonatos distintos. A unicidade é por:
>
> - `(player_id, badge_type_id, championship_id)` quando `scope = championship`
> - `(player_id, badge_type_id, year)` quando `scope = seasonal`
> - `(player_id, badge_type_id)` quando `scope = career` (único por toda a carreira)
>   Para badges de escopo `friendly`, a aplicação controla duplicidade via regra de negócio.

#### `team_badges` (novo — badges coletivos do time)
...
| `team_id`         | bigint FK | FK → `teams`                    |
| `badge_type_id`   | bigint FK | FK → `badge_types`              |
| `championship_id` | bigint FK | nullable — FK → `championships` |
| `awarded_at`      | timestamp |                                 |
| `notes`           | varchar   | nullable                        |

> Apenas badges de escopo `championship` com critério coletivo são atribuídos ao `team_badges` (ex: `unbeaten_champion`, `clean_sweep`). Badges individuais (`top_scorer`, `golden_ball`) ficam somente em `player_badges`.
> Ao encerrar um campeonato, o sistema atribui os badges coletivos simultaneamente em `player_badges` (para cada jogador em `championship_team_players`) e em `team_badges` (para o time).
...
erDiagram
    ...
    teams ||--o{ team_badges : "earns"
    badge_types ||--o{ team_badges : "typed as"
    championships ||--o{ team_badges : "origin"
```
...
| Tipo                 | Tabelas                                                                                                                                                                                                                                                                                                                                                                                                           |
| -------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Pivot puro           | `sport_mode_category`, `sport_mode_formation`, `sport_mode_position`, `championship_sport_modes`, `championship_team_players`                                                                                                                                                                                                                                                                                     |
| Entidade associativa | `team_sport_modes`, `team_staff`, `player_memberships`, `player_sport_preferences`, `championship_teams`, `championship_group_entries`, `championship_phases`, `championship_groups`, `championship_rounds`, `championship_matches`, `championship_match_highlights`, `championship_awards`, `team_invitations`, `friendly_matches`, `performance_highlights`, `player_badges`, `team_badges`, `team_stats_cache` |
...
Campo simples `users.plan` (enum) + `users.plan_expires_at` (timestamp nullable) para a Fase 4 do produto. Enquanto o cadastro de planos for manual/operacional, essa abordagem é suficiente para Gates e Policies.

Quando payment processing for integrado (Fase 3 produto), criar tabela `user_subscriptions` com histórico completo de assinaturas. O campo `users.plan` permanece como cache de leitura rápida.

### URL amigável (slug)

O campo `users.slug` (varchar(30), nullable, único) é a base para a URL `myclub.com.br/@slug`. Só exibido públicamente quando o usuário tem plano `player_pro` ou superior. O slug é definido pelo próprio usuário e validado para conter apenas caracteres alfanuméricos e hífen.

### Infraestrutura de notificações

**Fase 1–2 (atual):** notificações in-app via `Laravel Notifications` com canal `database`. A tabela `notifications` é criada automaticamente pelo Laravel — nenhuma migration manual necessária.

**Fase 8+:** adicionar tabela `device_tokens` (`id`, `user_id FK`, `token`, `platform enum (ios/android/web)`, `created_at`) para push notifications mobile. Não implementar antes da Fase 8.
